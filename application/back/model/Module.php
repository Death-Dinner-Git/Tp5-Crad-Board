...
use app\common\components\Tree;
...
class Module extends Model
{
    ...
    protected $_validate = array(
        array('name', 'require', '模块名称不能为空', 'regex', ),
        array('name', '', '该模块已存在', 'unique',),
        array('title', 'require', '模块标题不能为空', 'regex', ),
        array('description', 'require', '模块描述不能为空', 'regex', ),
        array('developer', 'require', '模块开发者不能为空', 'regex', ),
        array('version', 'require', '模块版本不能为空', 'regex', ),
        array('back_menu', 'require', '模块菜单节点不能为空', 'regex', ),
    );
     ...
     * @author Sir Fu
     */
    public function getBackMenu($module_name = '')
    {
        if ('' == $module_name) {
...
//        $back_menu = $this->getFieldByName(ucfirst($module_name), 'back_menu');
        $back_menu = [];
        foreach ($back_menu as $key => $val) {
            if (isset($val['url'])) {
    ...
    public function getAllMenu()
    {
        ...
        /**
         * @var \app\back\model\Access $user_group
         */
        $user_group = model('back/Access')->getFieldByUid($uid, 'group'); // 获得当前登录用户信息
        $group_auth = []; // 获得当前登录用户所属部门的权限列表
        ...
        if (!$menu_list || APP_DEBUG === true) {
            ...
            foreach ($system_module_list as $key => &$module) {
                $menu = json_decode($module['back_menu'], true);
    ...
    /**
     * 根据菜单ID的获取其所有父级菜单
     * @param array|string $current_menu 当前菜单信息
     * @param string $module_name 当前菜单信息
     * @return array 父级菜单集合
     * @author Sir Fu
     */
    ...
    public function getAll()
    {
        ...
        if ($installed_module_list) {
            foreach ($installed_module_list as &$module) {
                ...
                $new_module_list[$module['name']]['back_menu'] = json_decode($module['back_menu'], true);
            }

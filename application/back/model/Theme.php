        ...
        foreach ($theme_list as &$val) {
            switch ($val['status']) {
                case '-1': //未安装
                    ...
                    $val['right_button']['install']['attribute'] = 'class="label label-success-outline label-pill ajax-get" href="' . U('install', array('name' => $val['name'])) . '"';
                    break;
                default:
                    $val['status'] = '<i class="fa fa-check" style="color:green"></i>';
                    if ($val['current']) {
                        $val['right_button']['current']['title']     = '正在使用';
                        $val['right_button']['current']['attribute'] = 'class="label label-success label-pill" href="#"';
                    } else {
                        $val['right_button']['set_current']['title']     = '使用主题';
                        $val['right_button']['set_current']['attribute'] = 'class="label label-primary-outline label-pill ajax-get" href="' . U('setCurrent', array('id' => $val['id'])) . '"';
                    }

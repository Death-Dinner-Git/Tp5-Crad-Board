...
class BaseController extends Controller
{
    ...
    {
        $res = true;
        if ( Configs::getBackUser()->isGuest()){
            $res = false;
        }
        return $res;
    }
     ...
     * @param bool $refresh
     */
    protected function setCache($html, $userId, $route, $dependency, $root = null, $duration = 0, $refresh = false)
    {
        $tag = Config::$app->id;
        $key = [$userId, $route, $dependency, $root, $tag];
        $cache = Config::$app->cache;
        if ($duration !== 0 && intval($duration) === 0) {
            $duration = Config::$app->params['cacheDuration'];
        }
     ...
     * @return mixed
     */
    protected function getCache($userId, $route, $dependency, $root = null)
    {
        $tag = Config::$app->id;
        $key = [$userId, $route, $dependency, $root, $tag];
        $cache = Config::$app->cache;
...
//            }
     ...
     * @return bool
     */
    protected function deleteFolder($pathStr)
    {
        if (!is_dir($pathStr)) {
...
//            }
        ...
        //$d不是static目录下的文件不给予删除
        if (strstr($pathStr,'/static/') === false){
            return false;
        }
     ...
     */
    protected function getFolder($pathStr = './',$clear = false)
    {
//        if (strstr($pathStr, '/back/') !== false){
//            $pathStr = preg_replace('/\/back\//', '/backend/web/', $pathStr, 1); //只替换一次
//        }
...
//        }
    ...
    {
        return '<div style="width: 100%;height: 600px;display: flex;"><div style="margin: auto;">Would be stop run,when you run "'.$name.'" in the module. </div></div></div>';
    }
    ...
    public function setReturnUrl($url = null)
    {
        ...
        $path = session(config('user._back_url'));
        $path = (array) $path;
        if (end($path) != $url){
            $path[] = $url;
        }
        ...
        session(config('user._back_url'),$path);
    }
